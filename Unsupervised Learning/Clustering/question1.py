...
data = np.zeros((60, 2))
data[0:20, 0] = np.random.uniform(-3.5, -1.5, size=20)
data[0:20, 1] = np.random.uniform(-1, 0, size=20)

data[20:40, 0] = np.random.uniform(0, 4, size=20)
data[20:40, 1] = np.random.uniform(-3, -1, size=20)

data[40:60, 0] = np.random.uniform(0, 3, size=20)
data[40:60, 1] = np.random.uniform(0, 2, size=20)
...
for i in range(len(data)):
    ...
    plt.scatter(data[i][0], data[i][1], color=colors[cluster_label])
